...
in a variable.
```{r message = FALSE, warning = FALSE, echo = TRUE}
m14 = lm(rating~complaints,data=attitude)
```
...
```{r message = FALSE, warning = FALSE, echo = TRUE}
m15 = step(m14,.~.+privileges+learning+raises+critical+advance)
```
...
```{r message = FALSE, warning = FALSE, echo = TRUE}
# just the `learning` term was added
# AIC = 118.00 for the final model
summary(m15)
# Only the `complaints` term is significant in the final model - the intercept and coefficient of `learning` are not significant.
plot(m15)
# Despite the residuals not being perfectly Normally distributed, the model does seem reasonable.
```
