...
$t_{n-1} = t_{19} = \frac{\bar{X_{A-B}}}{s.e.(\bar{X_{A-B}})} =$
...
Given this test statistic, we need to compute the probability of observing such a statistic by chance. Such a probability is given by calculating the area under the curve defined by a particular distribution of t-statistics
...
- in this case we have a `t` distribution with `r df` degrees of freedom and a quantile of `r stat`
